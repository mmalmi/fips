...
- Repair NAT replay-window defaults and isolate discovery in topology and
  control-pressure test fixtures.
...
- Evict an end-to-end session after persistent AEAD failures outlive its
  recovery-rekey budget, allowing a clean handshake to replace poisoned state
  instead of consuming public-router receive-loop capacity indefinitely.
- Release `fips-core` and `fips-endpoint` 0.4.61 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Let an active authenticated bootstrap path initiate fresh direct UDP
  negotiation even after learned endpoint hints expire, keeping discovery
  addresses as optional probes rather than recovery prerequisites.
- Release `fips-core` and `fips-endpoint` 0.4.59 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Make packaged Linux health checks use the current required WSS seed URL
  interface and include the probe, wrapper, configuration example, and systemd
  units in generic install tarballs.
- Release `fips-core` and `fips-endpoint` 0.4.56 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Report the addresses of operational inbound UDP listeners independently of
  endpoint discovery and advertisement, so applications can verify socket
  ownership directly.
- Release `fips-core` and `fips-endpoint` 0.4.55 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Retire stale FMP rekey-drain epochs when their network carrier is replaced
  or declared dead, so rapid network changes and configured WebSocket
  reconnects can authenticate immediately instead of waiting on an obsolete
  drain window.
- Release `fips-core` and `fips-endpoint` 0.4.54 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Bound each ready Nostr discovery turn so authenticated payload, endpoint
  control queries, and graceful shutdown retain progress under sustained
  discovery pressure.
- Release `fips-core` and `fips-endpoint` 0.4.53 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Keep established sessions quiet after successful fallback-route recovery by
  suppressing recurring discovery while their indirect FSP owner is healthy
  and sendable, without delaying direct-path retries or recovery after that
  fallback becomes unusable.
- Release `fips-core` and `fips-endpoint` 0.4.52 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Reconnect configured WebSocket seeds promptly after an explicit network
  rebind instead of waiting for ordinary reconnect backoff.
- Reject an in-flight WebSocket handshake from the previous underlay before it
  can repopulate the cleared connection pool.
- Preserve established routed npub sessions and restore private payload
  delivery across the replacement WebSocket carrier.
- Release `fips-core` and `fips-endpoint` 0.4.50 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Invalidate stale FSP route and liveness evidence for every carrier rebuilt
  after a network change, preventing old fallback affinity from delaying
  payload recovery.
...
- Preserve the last authenticated mesh fallback while a recovered direct
  carrier performs bounded payload validation, so a later direct failure can
  immediately repin the established FSP session.
- Keep that fallback ownership across repeated direct-to-mesh-to-direct
  transitions without restarting the endpoint or changing the session.
- Release `fips-core` and `fips-endpoint` 0.4.47 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Stage a bounded direct FSP payload validation as soon as authenticated direct
  FMP control recovers, instead of retaining fallback payload affinity for the
  full direct-path degradation hold.
- Preserve the proven fallback route and its authenticated activity while that
  validation is staged, so traffic can immediately return to fallback if the
  recovered direct carrier does not authenticate payload.
- Release `fips-core` and `fips-endpoint` 0.4.46 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Make the simultaneous FSP role-change route regression deterministic by
  staging the losing setup first and waiting for the actual responder state
  while driving production-equivalent retransmit work.
- Release `fips-core` and `fips-endpoint` 0.4.45 together. Runtime code and
  FMP, FSP, discovery, routing, and physical transport wire formats are
  unchanged.
...
- Keep the first healthy same-carrier FMP connection as the canonical owner
  when duplicate same-direction dials complete concurrently, preventing the
  two peers from retaining mismatched Noise sessions.
- Renew direct-path degradation while authenticated replies continue through
  the selected fallback, preventing direct control refresh from periodically
  displacing a proven reply route under active traffic.
- Release `fips-core` and `fips-endpoint` 0.4.44 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Preserve the selected Linux or Android interface name in UDP bind failures,
  so callers can diagnose a disappeared underlay while the live carrier and
  session state roll back intact.
- Release `fips-core` and `fips-endpoint` 0.4.41 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Rebind configured UDP carriers onto the current interface after a network
  handoff without replacing the endpoint, authenticated peers, or FSP
  sessions.
- Retry in-flight direct handshakes immediately after a positive network
  change, reset an exhausted bounded retry budget, and let a newly
  authenticated tuple replace the preserved stale tuple.
- Preserve queued payload and carry it over an available mesh route while
  direct recovery continues, then release fallback affinity as soon as
  authenticated direct control and payload return.
- Release `fips-core` and `fips-endpoint` 0.4.40 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Evict authenticated clients as soon as their connection-oriented physical
  transport reports closure, instead of retaining dead WebSocket leaves in
  routing fanout for the generic 30-second silence timeout.
- Cover prompt WebSocket roster cleanup, TCP-reset recovery, persistent
  inter-seed replacement, and repeated fresh bidirectional route-by-npub
  delivery under client churn.
- Release `fips-core` and `fips-endpoint` 0.4.39 together. FMP, FSP,
  discovery, routing, and physical transport wire formats are unchanged.
...
- Replace an orphaned responder-side FMP rekey when the initiator exhausts a
  lost Msg2 exchange and retries with a fresh sender index, preventing repeated
  network roaming from leaving direct-path recovery permanently pending.
- Release `fips-core` and `fips-endpoint` 0.4.34 together. FMP, FSP,
...
- Keep direct-path retries pending until authenticated FSP payload validates
  the recovered path; FMP control heartbeats no longer cancel them early.
- Converge same-path roaming refreshes through the symmetric FMP rekey state
  machine, including stale carriers and simultaneous initiators.
- Prefer an authenticated direct carrier for a new local FSP session before
  inheriting stale routed affinity, while preserving normal cost-based routing
  after payload selects a branch.
- Release `fips-core` and `fips-endpoint` 0.4.31 together. FMP, FSP,
  discovery, routing semantics, and physical transport wire formats are
  unchanged.
...
- Admit an operator-configured WebSocket seed or listener adjacency under
  configured-only discovery policy. The identity ACL and deny list still run
  after this transport-level admission, while native and browser consumers can
  now share the authenticated first adjacency required by FSP-based WebRTC
  signaling in the upstream-compatible protocol.
- Release `fips-core` and `fips-endpoint` 0.4.20 together so embedded consumers
  receive the configured WebSocket bootstrap fix without version skew.
...
- Negotiate the newer direct-on-transport FSP carrier in previously reserved
  `SessionSetup` and `SessionAck` flag bits. Peers that do not advertise the
  capability, including upstream FIPS 0.4.1, automatically use its compatible
  FSP-inside-FMP carrier; modern peers retain the direct fast path.
...
- Pace repeated background direct-path upgrades at a deterministic jittered
  10–20 second cadence while a healthy routed fallback remains active. Actual
  link-dead recovery stays on its fast retry path, while unreachable static
  candidates no longer trigger a continuous handshake/discovery loop.
- Bump `fips-endpoint` to 0.4.7 so embedded consumers select the corrected
  `fips-core` release. FMP, FSP, discovery, datagram DFP1, and link-negotiation
...
- Restore the health probe's authenticated ICMPv6 echo over FSP instead of
...
- Resolved browser-emitted `.local` WebRTC ICE candidates with query-only mDNS
  while continuing to suppress native mDNS candidate gathering and its
  per-connection multicast listeners.
- Bumped `fips-endpoint` to 0.3.62 so app-facing consumers select
  `fips-core` 0.3.97.
...
- Bumped `fips-endpoint` to 0.3.59 so app-facing consumers select
  `fips-core` 0.3.92.
...
- Deferred first-session direct FSP ingress until authenticated handshake
  progress installs its route, avoiding a tight retry loop that could drop the
  first queued endpoint packet delivered alongside msg3.
...
- Published the dataplane direct dataplane endpoint APIs used by nvpn's direct TUN
  fast path, including app-facing endpoint data, packet run, packet batch, and
  sink types.

### Fixed

- Delivered direct endpoint packet batches before dataplane transport sends, reducing
  app-facing receive queue residence during bulk nvpn traffic.
- Deferred dataplane FSP ingress until local routes catch up, avoiding transient
  route-missing drops during reconnect or route churn.
- Bumped `fips-endpoint` to 0.3.53 so app-facing consumers pick up the
  `fips-core` 0.3.78 direct endpoint API and dataplane route-catch-up fixes.
