...
#[tokio::test]
async fn blocking_recv_packet_batch_for_each_respects_limit_without_public_vec_staging() {
    let endpoint = FipsEndpoint::builder()
        .without_system_tun()
        .bind()
        .await
    ...
    let endpoint = tokio::task::spawn_blocking(move || {
        let mut messages = Vec::with_capacity(3);
        let received = endpoint
            .blocking_recv_packet_batch_for_each(2, |message| {
                messages.push(message.data.as_ref().to_vec());
                true
            })
            .expect("messages should arrive");
        assert_eq!(received, 2);
        ...
        let received = endpoint
            .blocking_recv_packet_batch_for_each(8, |message| {
                messages.push(message.data.as_ref().to_vec());
                true
            })
            .expect("message should arrive");
...
#[tokio::test]
async fn blocking_recv_packet_batch_for_each_preserves_unhandled_internal_batch_tail() {
    let endpoint = FipsEndpoint::builder()
        .without_system_tun()
        .bind()
        .await
