//! Application-owned Nostr relay carrier for embedded FIPS endpoints.
//!
//! The adapter connects the ordinary FIPS relay transport to relays selected
//! by the embedding application. It does not own discovery, policy, or relay
//! configuration.
...
impl NostrRelayAdapter {
    /// Connect an embedded endpoint through the application-provided relays.
    pub async fn start(
        endpoint: Arc<FipsEndpoint>,
        relays: &[String],
    ...
    /// Connect a direct [`crate::Node`] through its attached relay I/O.
    ...
    /// Stop the carrier and close its application-owned relay client.
