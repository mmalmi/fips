...
async fn test_disconnect_chain_peer_removal() {
    ...
    nodes[1]
        .node
        .send_dataplane_fmp_link_plaintext(&node0_addr, &plaintext, false)
        .await
...
async fn test_disconnect_star_hub_departs() {
    ...
    for spoke_idx in 1..4 {
        ...
        nodes[0]
            .node
            .send_dataplane_fmp_link_plaintext(&spoke_addr, &plaintext, false)
            .await
            .expect("Failed to send disconnect");
    }
...
async fn test_disconnect_clears_session() {
    ...
    nodes[0]
        .node
        .send_dataplane_fmp_link_plaintext(&node1_addr, &disconnect.encode(), false)
        .await
        .expect("Failed to send disconnect");
