...
/// Smoke test: handshake completes correctly with `actor_owns_peer` on.
///
/// Validates that `promote_connection` ships the freshly-promoted
/// `ActivePeer` to its per-peer actor task (peer ends up in
/// `peer_actors` not `peers`), and that the recall/reship cycle round-
/// trips cleanly. Also exercises `remove_active_peer`'s actor-aware
/// cleanup (drops `peer_actors` entry, sends `RemovePeer`).
#[tokio::test]
async fn test_actor_owns_peer_handshake_lifecycle() {
    use crate::config::UdpConfig;
    ...
    // Node B promoted on msg1. Peer A should be in B's actor, not in
    // B.peers.
    let peer_a_node_addr =
        *PeerIdentity::from_pubkey_full(node_a.identity.pubkey_full()).node_addr();
    assert_eq!(
        node_b.peers.len(),
        0,
        "actor_owns_peer: B.peers should be empty after promote"
    );
    assert_eq!(
        node_b.peer_actors.len(),
        1,
        "actor_owns_peer: B.peer_actors should hold the freshly promoted peer"
    );
    assert!(node_b.peer_actors.contains_key(&peer_a_node_addr));
    assert_eq!(node_b.peer_count(), 1, "peer_count covers actor-owned peers");
    ...
    // Same on Node A after msg2 completes the handshake.
    assert_eq!(node_a.peers.len(), 0);
    assert_eq!(node_a.peer_actors.len(), 1);
    assert!(node_a.peer_actors.contains_key(&peer_b_node_addr));

    // Recall + reship cycle (same pattern as the tick handler).
    node_a.recall_all_actor_peers().await;
    assert_eq!(
        node_a.peers.len(),
        1,
        "after recall, peer is back in Node.peers"
    );
    assert_eq!(node_a.peer_actors.len(), 0);
    assert_eq!(node_a.recalled_handles.len(), 1);

    node_a.reship_all_actor_peers();
    assert_eq!(
        node_a.peers.len(),
        0,
        "after reship, peer is back in actor"
    );
    assert_eq!(node_a.peer_actors.len(), 1);
    assert_eq!(node_a.recalled_handles.len(), 0);

    // Cleanup: remove_active_peer drops peer_actors entry and the
    // actor task exits as its handle clones drop.
    node_a.remove_active_peer(&peer_b_node_addr);
    assert_eq!(node_a.peer_actors.len(), 0);
    assert_eq!(node_a.peers.len(), 0);
...
/// `find_next_hop` and `send_endpoint_data` work against an actor-owned
/// direct peer.
///
/// Caught the "no route to destination" bug — `find_next_hop`'s direct-
/// peer check did `self.peers.get(addr)` only, missing actor-owned
/// peers. SessionAck and SessionSetup outbound paths fell through to
/// the routing layer's tree/bloom code (also iterates `self.peers`),
/// returning None → endpoint sends silently failed → mesh never got
/// to "connected".
#[tokio::test]
async fn test_actor_owns_peer_find_next_hop_and_endpoint_send() {
    use crate::config::UdpConfig;
    use crate::node::wire::build_msg1;
    use crate::transport::udp::UdpTransport;
    use tokio::time::{Duration, timeout};
    ...
    // Peer is now in A's actor. find_next_hop must still resolve it
    // as a direct peer (legacy `self.peers.get(addr)` would miss).
    let next_hop = node_a.find_next_hop(&peer_b_node_addr);
    assert_eq!(
        next_hop,
        Some(peer_b_node_addr),
        "find_next_hop must return the direct peer even when actor-owned (peer_actors keys: {:?})",
        node_a.peer_actors.keys().collect::<Vec<_>>()
    );
