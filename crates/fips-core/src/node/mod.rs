...
pub struct Node {
    ...
    // === Peers (Active Phase) ===
    /// Authenticated peers — plain owned `ActivePeer`. Single owner
    /// (rx_loop's `&mut self.peers[addr]`). The peer-actor task does
    /// not share access to `ActivePeer`; it handles only session work
    /// on its owned `SessionEntry`. The previous
    /// `Arc<RwLock<ActivePeer>>` wrapper is gone (step 7d).
    ...
    /// no RwLock. With `node.actor_owns_sessions = true` direct-peer
    /// sessions live in their peer actor's owned state; this map only
    /// holds transit-endpoint sessions (rare 3+-hop endpoint case).
    /// With the flag off, every session lives here. Single owner.
...
impl Node {
    ...
    pub(crate) fn peer_display_name(&self, addr: &NodeAddr) -> String {
        ...
        if let Some(slot) = self.peers.get(addr) {
            return slot.identity().short_npub();
        }
    ...
    pub(crate) fn cleanup_bootstrap_transport_if_unused(&mut self, transport_id: TransportId) {
        ...
        let transport_in_use = self
            ...
            || self.peers.values().any(|slot| {
                slot.transport_id() == Some(transport_id)
            })
            || self
                .pending_connects
                .iter()
    ...
    /// Get a mutable session by remote NodeAddr (test-only).
    #[cfg(test)]
    ...
    pub(crate) fn session_entries(
        &self,
    ) -> impl Iterator<Item = (&NodeAddr, &SessionEntry)> {
        self.sessions.iter()
    }
    ...
    pub fn find_next_hop(&mut self, dest_node_addr: &NodeAddr) -> Option<NodeAddr> {
        ...
        let tree_route_addr = self
            ...
            .filter(|next_hop_id| {
                self.peers
                    .get(next_hop_id)
                    .is_some_and(|slot| slot.can_send())
            });
    ...
    pub fn destination_in_filters(&self, dest: &NodeAddr) -> Vec<NodeAddr> {
        self.peers
            .iter()
            .filter(|(_, slot)| slot.may_reach(dest))
            .map(|(addr, _)| *addr)
            .collect()
    }
    ...
    async fn send_encrypted_link_message_inner(
        ...
        // Single &mut borrow across the encrypt step (mutates
        // send_cipher counter via NoiseSession).
