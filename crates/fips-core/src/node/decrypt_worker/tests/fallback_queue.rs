    ...
    #[test]
    fn accepted_fsp_registration_can_colocate_with_preferred_fmp_owner() {
        let (pool, priority_receivers, bulk_receivers) =
            test_worker_pool(4, DECRYPT_WORKER_BULK_BATCH_MAX);
        ...
        let preferred_owner = (default_owner + 1) % priority_receivers.len();

        assert!(
            pool.register_fsp_session(
                source_addr,
                dummy_fsp_recv_snapshot(&source, &local),
                Some(preferred_owner),
            ),
            "FSP registration should accept the preferred FMP receive owner"
        );
        assert_eq!(
            pool.worker_idx_for_fsp(&source_addr),
            preferred_owner,
            "accepted preferred owner should override the independent FSP hash"
        );
        match priority_receivers[preferred_owner]
            .try_recv()
            .expect("preferred owner should receive FSP registration")
        {
            WorkerMsg::RegisterFspSession {
                source_addr: queued,
                ..
            } => assert_eq!(queued, source_addr),
            WorkerMsg::Job(_)
            | WorkerMsg::FspJob(_)
            | WorkerMsg::RegisterSession { .. }
            | WorkerMsg::UnregisterSession { .. }
            | WorkerMsg::UnregisterFspSession { .. } => {
                panic!("expected preferred-owner FSP registration")
            }
        }
        ...
        job.source_addr = source_addr;
        assert!(
            pool.dispatch_fsp_job_or_return(job).is_ok(),
            "bulk FSP job should dispatch to accepted preferred owner"
        );
        match bulk_receivers[preferred_owner]
            .try_recv()
            .expect("preferred owner should receive FSP job")
        {
            DecryptWorkerBulkItem::FspJob(job) => assert_eq!(job.source_addr, source_addr),
            DecryptWorkerBulkItem::Job(_)
            | DecryptWorkerBulkItem::Batch(_)
            | DecryptWorkerBulkItem::FspBatch(_) => panic!("expected FSP job"),
        }
        ...
        assert!(
            ...
            "second registration should accept the moved owner"
        );
        match priority_receivers[default_owner]
            .try_recv()
            .expect("new owner should receive replacement FSP registration")
        {
            WorkerMsg::RegisterFspSession {
                source_addr: queued,
                ..
            ...
            | WorkerMsg::UnregisterSession { .. }
            | WorkerMsg::UnregisterFspSession { .. } => {
                panic!("expected moved-owner FSP registration")
            }
        }
        match priority_receivers[preferred_owner]
            .try_recv()
            .expect("old owner should receive stale FSP unregister")
        {
            WorkerMsg::UnregisterFspSession {
                source_addr: queued,
            } => assert_eq!(queued, source_addr),
            ...
            | WorkerMsg::RegisterFspSession { .. }
            | WorkerMsg::UnregisterSession { .. } => {
                panic!("expected stale-owner FSP unregister")
            }
        }
    }
