    ...
    #[test]
    fn bulk_fmp_worker_open_accepts_replay_only_on_ordered_completion() {
        let key_bytes = [0x42u8; 32];
        let seal_cipher = test_chacha_key(key_bytes);
        let open_cipher = test_chacha_key(key_bytes);
        let session_key = test_session_key(9, 80);
        let source_peer = test_source_peer();
        let (pool, _control_receivers, _priority_receivers, bulk_receivers, aead_receivers) =
        ...
        let processed = handle_bulk_item(
            ...
            &priority_rx,
            &aead_receivers[0],
            DecryptWorkerBulkItem::Job(decrypt_job_for_test_packet(
                packet,
                header,
                session_key,
                counter,
        ...
        assert_eq!(
            shard.fmp_replay_highest(session_key).unwrap(),
            0,
            "owner must not advance FMP replay before opener completion"
        );
        ...
        for (idx, rx) in bulk_receivers.iter().enumerate() {
            if let Ok(item) = rx.try_recv() {
                match item {
                    ...
                    _ => panic!("expected one FMP opener job"),
                }
            }
        }
        let opener = opener.expect("bulk FMP packet should queue one opener job");
        let open_idx = open_idx.expect("opener worker index should be captured");
        complete_fmp_aead_open_job(open_idx, &pool, opener);

        let completion = aead_receivers[0]
            .try_recv()
            .expect("opener completion should return to owner");
        let completion = match completion {
            AeadCompletionBatch::Fmp(completion) => completion,
            AeadCompletionBatch::Fsp(_) => panic!("expected FMP opener completion"),
        };
        shard.handle_fmp_aead_completion_batch_msg(0, completion, &mut plaintext_batch);
        ...
        assert!(
            ...
            "opened non-session FMP payload should fall back after ordered replay accept"
        );
        assert_eq!(
            shard.fmp_replay_highest(session_key).unwrap(),
            counter,
            "ordered opener completion must advance the owner replay window"
        );
    }
