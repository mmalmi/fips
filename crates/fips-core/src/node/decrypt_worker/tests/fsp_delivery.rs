    ...
    fn fsp_session_refresh_classifies_old_worker_open_aead_as_stale_completion() {
        ...
        assert_eq!(
            ...
            "old-K worker-open failures after refresh must not count as hard AEAD"
        );
    ...
    fn fsp_session_refresh_classifies_same_k_old_worker_open_aead_as_stale_completion() {
        ...
        refreshed.attach_shared_crypto_session(new_shared);
        ...
        shard.handle_fsp_aead_completion_msg(
            0,
            FspAeadCompletion {
                source_addr,
                receive_order_id,
        ...
        assert!(
            ...
            "stale same-K worker-open completion must not report an AEAD failure"
        );
        let state = shard
            .fsp_sessions
            .get(&source_addr)
            .expect("FSP session should remain registered");
