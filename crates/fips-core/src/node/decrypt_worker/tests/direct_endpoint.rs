    ...
    fn decrypt_worker_bulk_drain_budget_matches_receive_batch_width() {
        ...
        assert_eq!(
            ...
            DECRYPT_WORKER_BULK_BATCH_MAX,
            "completion interleave should be bounded to one bulk batch width"
        );
    ...
    #[test]
    fn decrypt_worker_bulk_batch_bounds_aead_completion_interleave() {
        ...
        assert_eq!(
            fsp_aead_completion_rx.len(),
            3,
            "a saturated completion lane should drain one bounded slice per bulk batch"
        );
    }
    ...
    #[test]
    fn decrypt_worker_fsp_bulk_batch_bounds_aead_completion_interleave() {
        let mut shard = test_shard();
        let (_control_tx, control_rx) = bounded::<WorkerMsg>(1);
        let (_priority_tx, priority_rx) = bounded::<WorkerMsg>(1);
        ...
        assert_eq!(
            fsp_aead_completion_rx.len(),
            5,
            "FSP owner bulk service should drain one bounded completion slice per bulk batch"
        );
    }
