...
impl DecryptWorkerPool {
    ...
    pub(crate) fn spawn_with_direct_delivery_sink(
        ...
        for _ in 0..n {
            ...
            let (fmp_aead_completion_tx, fmp_aead_completion_rx) =
                bounded::<FmpAeadCompletion>(fsp_aead_completion_channel_cap_from_bulk_cap(
                    bulk_channel_cap,
                ));
    ...
    fn send_fmp_aead_completion(&self, owner_idx: usize, completion: FmpAeadCompletion) -> bool {
        self.senders
            .get(owner_idx)
    ...
    fn dispatch_fmp_aead_open_worker_job(
            ...
            .try_send(DecryptWorkerBulkItem::FmpAeadOpen(job))
        {
            ...
            Err(TrySendError::Disconnected(DecryptWorkerBulkItem::FmpAeadOpen(job))) => {
                ...
                debug!(
                    worker = open_idx,
                    "DecryptWorker FMP opener thread gone; dropping FMP opener job"
                );
                Err(job)
            }
