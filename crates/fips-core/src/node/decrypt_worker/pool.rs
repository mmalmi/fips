    ...
    fn spawn(n: usize, channel_cap: usize) -> Option<Arc<Self>> {
        ...
        for i in 0..n {
            ...
            std::thread::Builder::new()
                .name(format!("fips-fsp-aead-{i}"))
                .spawn(move || run_fsp_aead_helper(i, helper_rx))
                .expect("failed to spawn fips-fsp-aead OS thread");
        }
        info!(helpers = n, "Spawned FSP AEAD helper pool");
...
fn run_fsp_aead_helper(idx: usize, rx: Receiver<FspAeadHelperJob>) {
    ...
    while let Ok(mut job) = rx.recv() {
        crate::perf_profile::record_since_count(
            crate::perf_profile::Stage::DecryptFspAeadHelperQueueWait,
