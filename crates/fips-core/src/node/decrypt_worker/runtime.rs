...
fn drain_aead_completions_for_bulk_item(
    ...
    if started_with_budget > 0
        && *remaining_budget == 0
        && !fsp_aead_completion_rx.is_empty()
    {
...
fn handle_bulk_item(
    ...
    match item {
        ...
        DecryptWorkerBulkItem::FspAeadOpen(job) => {
            ...
            crate::perf_profile::record_fsp_aead_open_worker_target(idx, 1);
        ...
        DecryptWorkerBulkItem::FspAeadOpenBatch(jobs) => {
            ...
            crate::perf_profile::record_fsp_aead_open_worker_target(idx, count);
