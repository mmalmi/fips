...
fn complete_fsp_aead_open_job(idx: usize, pool: &DecryptWorkerPool, job: FspAeadOpenJob) {
    complete_fsp_aead_open_job_to_owner(idx, pool, job);
}

fn complete_fsp_aead_open_job_to_owner(idx: usize, pool: &DecryptWorkerPool, mut job: FspAeadOpenJob) {
...
fn complete_fsp_aead_open_jobs(idx: usize, pool: &DecryptWorkerPool, jobs: Vec<FspAeadOpenJob>) {
...
fn handle_bulk_item_with_buffers(
    ...
    match item {
        ...
        DecryptWorkerBulkItem::FspAeadOpen(job) => {
            let item_service_started_at = crate::perf_profile::stamp();
            complete_fsp_aead_open_job_to_owner(idx, &shard.pool, job);
            record_decrypt_worker_bulk_item_service(item_service_started_at, 1);
            1
        }
