    ...
    #[tokio::test]
    async fn worker_direct_session_commit_batch_does_not_flush_pending_for_stale_commits() {
        let local = Identity::generate();
        let peer = Identity::generate();
        let source_peer = PeerIdentity::from_pubkey_full(peer.pubkey_full());
        let source_addr = *peer.node_addr();
        ...
        node.process_direct_session_commit_batch_from_worker(vec![commit(10), commit(10)])
            .await;

        assert!(
            node.pending_session_traffic.has_traffic_for(&source_addr),
            "stale direct commits must not flush queued traffic for their source"
        );
        assert!(
            endpoint_io.event_rx.try_recv().is_err(),
            "queued endpoint data should not be released by stale commits"
        );
        let entry = node
            .sessions
            .get(&source_addr)
            .expect("session should remain");
        assert_eq!(entry.current_highest_counter(), Some(10));
    }
