...
mod tests {
    ...
    fn session_registry_owns_due_session_mmp_report_collection() {
        ...
        let now = Instant::now();
        let mut entry = session_entry_with_mmp_config(
            &local,
            &peer,
            &crate::config::SessionMmpConfig::default(),
        );
    ...
    fn session_registry_session_mmp_report_collection_respects_modes() {
        ...
        let mut lightweight_config = crate::config::SessionMmpConfig::default();
        ...
        let mut minimal_config = crate::config::SessionMmpConfig::default();
    ...
    fn session_registry_owns_session_mmp_send_result_accounting() {
        ...
        let peer = Identity::generate();
        let peer_addr = *peer.node_addr();
        let entry = session_entry_with_mmp_config(
            &local,
            &peer,
            &crate::config::SessionMmpConfig::default(),
        );
    ...
    fn peer_lifecycle_registry_owns_link_heartbeat_planning_and_sent_bookkeeping() {
        ...
        let initial =
            peers.plan_link_heartbeat_tick(now, Duration::from_secs(10), 3, false, |_| {
    ...
    fn peer_lifecycle_registry_owns_link_dead_and_deferred_heartbeat_planning() {
        ...
        let deferred =
            peers.plan_link_heartbeat_tick(now, Duration::from_secs(10), 3, true, |_| {
                Duration::from_secs(30)
            });
