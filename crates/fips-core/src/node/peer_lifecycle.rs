...
impl PeerLifecycleRegistry {
    ...
    pub(in crate::node) fn record_authenticated_fmp_receive_batch<I>(
        &mut self,
        node_addr: &NodeAddr,
        transport_id: TransportId,
        remote_addr: &TransportAddr,
    ...
    where
        I: IntoIterator<Item = AuthenticatedFmpReceiveRecord>,
    {
        let peer = self.active.get_mut(node_addr)?;
        peer.reset_decrypt_failures();
        ...
        let mut last_timestamp_ms = 0u64;
        {
            ...
            for record in records {
                ...
                last_timestamp_ms = record.packet_timestamp_ms;
                if let Some(mmp) = mmp.as_mut() {
                    mmp.receiver.record_recv(
                        record.fmp_counter,
                        record.inner_timestamp_ms,
                        record.packet_len,
                        record.ce_flag,
                        now,
                    );
                    if let Some(rtt) =
                        mmp.spin_bit
                            .rx_observe(record.sp_flag, record.fmp_counter, now)
                    {
                        result.spin_rtt = Some(rtt);
                    }
                    result.mmp_recorded = true;
                }
            }
        ...
        if packets > 0 {
            peer.link_stats_mut()
                .record_recv_batch(packets, bytes, last_timestamp_ms);
            peer.touch(last_timestamp_ms);
        }
