...
impl EncryptWorkerPool {
    ...
    fn prepare_dispatch(&self, job: FmpSendJob) -> (usize, QueuedFmpSendJob) {
        ...
        let ticket = self
            ...
            / linux_worker_stride();
        let idx = ticket % self.senders.len();
        (idx, QueuedFmpSendJob::linux_sequenced(job, flow))
    }
...
fn linux_ordered_sender_enabled() -> bool {
    // Experimental WireGuard-go-shaped sender: keep control/liveness on the
    // direct priority lane, but spread bulk FMP/FSP AEAD across workers and
    // serialize completed packets through one ordered sender per kernel target.
...
#[cfg(target_os = "linux")]
fn linux_ordered_pending_cap() -> usize {
    static VALUE: OnceLock<usize> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_ORDERED_PENDING_CAP")
            .ok()
            .and_then(|raw| raw.trim().parse::<usize>().ok())
            .unwrap_or(1024)
            .clamp(64, 32768)
...
impl LinuxSequencedSendFlow {
    ...
    fn complete_many(&self, items: Vec<(u64, LinuxSendItem)>) {
        ...
        let pending_cap = linux_ordered_pending_cap();
        let mut state = self.state.lock().expect("linux send flow state poisoned");
        if state.closed {
            return;
        }
    ...
    fn take_ready_groups(&self) -> Option<Vec<SelectedSendBatch>> {
        let max_packets = linux_ordered_send_batch_size();
        let mut state = self.state.lock().expect("linux send flow state poisoned");
        loop {
    ...
    fn run(self: Arc<Self>) {
        ...
        while let Some(groups) = self.take_ready_groups() {
