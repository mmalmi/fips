...
pub(crate) struct EncryptWorkerPool {
    ...
    #[cfg(target_os = "linux")]
    linux_batch: Option<Arc<LinuxSequencedBatchPool>>,
    #[cfg(target_os = "macos")]
...
impl EncryptWorkerPool {
    ...
    pub fn spawn(n: usize) -> Self {
        ...
        #[cfg(target_os = "linux")]
        let linux_batch = linux_batch_sequenced_sender_enabled()
            .then(|| Arc::new(LinuxSequencedBatchPool::spawn(n)));
    ...
    pub(crate) fn dispatch_bulk_batch(&self, jobs: Vec<FmpSendJob>) {
        #[cfg(target_os = "linux")]
        let jobs = if let Some(batch_pool) = self.linux_batch.as_ref() {
            match batch_pool.try_dispatch_batch(jobs) {
                Ok(()) => return,
                Err(jobs) => jobs,
            }
        } else {
...
#[cfg(target_os = "linux")]
struct LinuxSequencedBatchPool {
    senders: Arc<[Sender<LinuxSequencedBatchJob>]>,
    flows: Arc<LinuxSequencedSendFlows>,
    next_worker: std::sync::atomic::AtomicUsize,
}

#[cfg(target_os = "linux")]
impl LinuxSequencedBatchPool {
    fn spawn(n: usize) -> Self {
        let n = n.max(1);
        let cap = linux_batch_channel_cap();
        let flows = Arc::new(LinuxSequencedSendFlows::default());
        let mut senders = Vec::with_capacity(n);
        for i in 0..n {
            let (tx, rx) = bounded(cap);
            std::thread::Builder::new()
                .name(format!("fips-linux-batch-encrypt-{i}"))
                .spawn(move || run_linux_batch_crypto_worker(i, rx))
                .expect("failed to spawn fips linux batch encrypt thread");
            senders.push(tx);
        }
        Self {
            senders: senders.into(),
    ...
    fn try_dispatch_batch(&self, jobs: Vec<FmpSendJob>) -> Result<(), Vec<FmpSendJob>> {
        if !linux_batch_jobs_eligible(&jobs) {
            return Err(jobs);
        }
        let packet_count = jobs.len();
        let flow = self.flows.flow_for(&jobs[0]);
        let seq = flow.reserve_seq();
        let ticket = self
            .next_worker
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed)
            / linux_batch_worker_stride();
        let idx = ticket % self.senders.len();
        match self.senders[idx].try_send(LinuxSequencedBatchJob { flow, seq, jobs }) {
            Ok(()) => Ok(()),
            Err(TrySendError::Full(job)) => {
                record_linux_batch_drop(idx, packet_count);
                job.flow.complete(job.seq, LinuxSendBatchItem::Skip);
                Ok(())
            }
...
#[cfg(target_os = "linux")]
fn linux_batch_jobs_eligible(jobs: &[FmpSendJob]) -> bool {
    if jobs.len() <= 1 {
        return false;
    }
...
#[cfg(target_os = "linux")]
fn record_linux_batch_drop(worker: usize, packet_count: usize) {
    for _ in 0..packet_count {
        record_encrypt_worker_queue_full(EncryptWorkerLane::Bulk);
        record_encrypt_worker_backpressure_drop(worker);
    }
}

#[cfg(target_os = "linux")]
fn linux_batch_sequenced_sender_enabled() -> bool {
    static VALUE: OnceLock<bool> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_BATCH_SEQUENCED_SENDER")
            .ok()
            .map(|raw| {
                !matches!(
...
#[cfg(target_os = "linux")]
fn linux_batch_channel_cap() -> usize {
    static VALUE: OnceLock<usize> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_BATCH_CHANNEL_CAP")
            .ok()
...
#[cfg(target_os = "linux")]
fn linux_batch_worker_stride() -> usize {
    static VALUE: OnceLock<usize> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_BATCH_WORKER_STRIDE")
            .ok()
            .and_then(|raw| raw.trim().parse::<usize>().ok())
            .unwrap_or(1)
            .clamp(1, 64)
...
#[cfg(target_os = "linux")]
impl LinuxSequencedBatchJob {
    fn seal_and_complete(self) {
        let Self { flow, seq, jobs } = self;
        let packet_count = jobs.len();
        if packet_count == 0 {
            flow.complete(seq, LinuxSendBatchItem::Skip);
            return;
        }
        crate::perf_profile::record_fmp_worker_batch( packet_count, 0, packet_count, worker_batch_size());
        let mut sealed_packets = Vec::with_capacity(packet_count);
        {
            let _t = crate::perf_profile::Timer::start(crate::perf_profile::Stage::FmpEncrypt);
            for job in jobs {
                let target_key = job.send_target_key();
                match SealedSendPacket::from_job_with_target_key(job, target_key) {
                    Ok(sealed) => sealed_packets.push(sealed),
                    Err(_) => {}
                }
            }
        }
        flow.complete(seq, LinuxSendBatchItem::Packets(sealed_packets));
    }
}

#[cfg(target_os = "linux")]
fn run_linux_batch_crypto_worker(idx: usize, rx: Receiver<LinuxSequencedBatchJob>) {
    trace!(worker = idx, "FIPS Linux batch encrypt worker thread starting");
    while let Ok(job) = rx.recv() {
        job.seal_and_complete();
    }
    trace!(worker = idx, "FIPS Linux batch encrypt worker thread exiting");
}

#[cfg(target_os = "linux")]
#[derive(Default)]
struct LinuxSequencedSendFlows {
    flows: Mutex<HashMap<SendTargetKey, Arc<LinuxSequencedSendFlow>>>,
...
impl LinuxSequencedSendFlows {
    fn flow_for(&self, job: &FmpSendJob) -> Arc<LinuxSequencedSendFlow> {
        ...
        let flow = LinuxSequencedSendFlow::spawn(key, now_ms);
    ...
    fn prune_idle_locked(
        &self,
        flows: &mut HashMap<SendTargetKey, Arc<LinuxSequencedSendFlow>>,
        now_ms: u64,
    ) {
        let last = self
            .last_prune_ms
        ...
        let idle_ms = linux_batch_send_flow_idle_ms();
        flows.retain(|_, flow| {
...
#[cfg(target_os = "linux")]
impl Drop for LinuxSequencedSendFlows {
    fn drop(&mut self) {
        let flows = self.flows.lock().expect("linux send flow map poisoned");
...
struct LinuxSequencedSendFlow {
    ...
    state: Mutex<LinuxSequencedSendFlowState>,
    ready_cv: Condvar,
    space_cv: Condvar,
}
...
#[cfg(target_os = "linux")]
#[derive(Default)]
struct LinuxSequencedSendFlowState {
    next_send_seq: u64,
    pending: BTreeMap<u64, LinuxSendBatchItem>,
    closed: bool,
}

#[cfg(target_os = "linux")]
enum LinuxSendBatchItem {
...
impl LinuxSequencedSendFlow {
    fn spawn(key: SendTargetKey, now_ms: u64) -> Arc<Self> {
        let flow = Arc::new(Self {
            key,
            next_seq: std::sync::atomic::AtomicU64::new(0),
            last_used_ms: std::sync::atomic::AtomicU64::new(now_ms),
            state: Mutex::new(LinuxSequencedSendFlowState::default()),
        ...
        std::thread::Builder::new()
            ...
            .expect("failed to spawn fips Linux sequenced send thread");
        flow
    }
    ...
    fn complete(&self, seq: u64, item: LinuxSendBatchItem) {
        const PENDING_CAP: usize = 4096;
        let mut state = self.state.lock().expect("linux send flow state poisoned");
        if state.closed {
            return;
        }
        while state.pending.len() >= PENDING_CAP && seq != state.next_send_seq {
            state = self
                .space_cv
                .wait(state)
    ...
    fn run(self: Arc<Self>) {
        trace!(
            ...
            dest = %self.key.dest_addr,
            "Linux sequenced UDP sender starting"
        );
        loop {
            let item = {
                let mut state = self.state.lock().expect("linux send flow state poisoned");
            ...
            match item {
                LinuxSendBatchItem::Packets(packets) => {
                    if let Err(err) = flush_linux_sealed_packets_sync(packets) {
                        debug!(
                            socket_fd = self.key.socket_fd,
                            connected_fd = ?self.key.connected_fd,
                            dest = %self.key.dest_addr,
                            error = %err,
                            "Linux sequenced UDP send failed"
                        );
                    }
                }
                LinuxSendBatchItem::Skip => {}
            }
        }
    }
}
...
#[cfg(target_os = "linux")]
fn linux_batch_send_flow_idle_ms() -> u64 {
    static VALUE: OnceLock<u64> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_BATCH_SEND_FLOW_IDLE_MS")
            .ok()
