...
fn linux_ordered_sender_enabled() -> bool {
    // Architecture probe only. This mirrors wireguard-go's
    // parallel-crypto/ordered-send split, but the first Docker A/Bs showed
    // the per-packet BTreeMap completion lane is too expensive for Linux
    // no-direct traffic. Keep it opt-in while we iterate on chunk/container
    // completion rather than enabling this shape by accident.
...
#[cfg(target_os = "linux")]
fn linux_ordered_sender_batch_size() -> usize {
    static VALUE: OnceLock<usize> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_ORDERED_SENDER_BATCH")
            .ok()
            .and_then(|raw| raw.trim().parse::<usize>().ok())
...
#[cfg(target_os = "linux")]
fn linux_send_flow_pending_cap() -> usize {
    static VALUE: OnceLock<usize> = OnceLock::new();
    *VALUE.get_or_init(|| {
        std::env::var("FIPS_LINUX_ORDERED_SENDER_PENDING_CAP")
            .ok()
            .and_then(|raw| raw.trim().parse::<usize>().ok())
            .unwrap_or(4096)
            .clamp(1, 32768)
...
impl LinuxSequencedSendFlow {
    ...
    fn complete_many(&self, items: Vec<(u64, LinuxSendItem)>) {
        ...
        let pending_cap = linux_send_flow_pending_cap();
        let mut state = self.state.lock().expect("linux send flow state poisoned");
        if state.closed {
            return;
        }
    ...
    fn run(self: Arc<Self>) {
        ...
        let batch_size = linux_ordered_sender_batch_size();
        ...
        loop {
            ...
            self.flush_ready_items(items, batch_size);
        }
    }

    fn flush_ready_items(&self, items: Vec<LinuxSendItem>, batch_size: usize) {
