...
mod tests {
    ...
    #[test]
    fn linux_batch_seal_chunk_preserves_indices_and_wire_lengths() {
        use crate::transport::udp::socket::UdpRawSocket;
        use ring::aead::{LessSafeKey, UnboundKey};
        ...
        rt.block_on(async {
            ...
            let result = linux_batch_seal_chunk(
                41,
                vec![
                    make_job(send_sock.clone(), &cipher, 1, dest, 32),
                    make_job(send_sock.clone(), &cipher, 2, dest, 64),
                ],
            );
            assert_eq!(result.sealed.len(), 2);
            assert_eq!(result.sealed[0].0, 41);
            assert_eq!(result.sealed[1].0, 42);
            assert_eq!(result.sealed[0].1.wire_packet().len(), 16 + 32 + 16);
            assert_eq!(result.sealed[1].1.wire_packet().len(), 16 + 64 + 16);
        });
    }
}
