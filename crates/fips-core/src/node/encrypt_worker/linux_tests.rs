...
mod tests {
    ...
    #[test]
    fn linux_batch_sequenced_sender_accepts_only_bulk_single_target_batches() {
        use crate::transport::udp::socket::UdpRawSocket;
        use ring::aead::{LessSafeKey, UnboundKey};

        fn cipher() -> LessSafeKey {
        ...
        fn job(
            ...
            let mut wire_buf = Vec::with_capacity(ESTABLISHED_HEADER_SIZE + 16 + 16);
            ...
            FmpSendJob {
                ...
                wire_buf,
                fsp_seal: None,
                send_target: SelectedSendTarget::new(socket, None, dest),
                bulk_endpoint_data,
        ...
        rt.block_on(async {
            ...
            let socket = raw.into_async().expect("into_async");
            let cipher = cipher();
            let dest_a: SocketAddr = "127.0.0.1:10052".parse().unwrap();
            let dest_b: SocketAddr = "127.0.0.1:10053".parse().unwrap();

            assert!(linux_batch_jobs_eligible(&[
                job(socket.clone(), &cipher, dest_a, true),
                job(socket.clone(), &cipher, dest_a, true),
            ]));
            assert!(!linux_batch_jobs_eligible(&[job(
                socket.clone(),
                &cipher,
                dest_a,
                true,
            ...
            assert!(!linux_batch_jobs_eligible(&[
                job(socket.clone(), &cipher, dest_a, true),
                job(socket.clone(), &cipher, dest_a, false),
            ]));
            assert!(!linux_batch_jobs_eligible(&[
                job(socket.clone(), &cipher, dest_a, true),
                job(socket, &cipher, dest_b, true),
            ]));
        });
    }

    #[test]
    fn linux_sequenced_send_flow_sends_completed_batches_in_sequence_order() {
        use crate::transport::udp::socket::UdpRawSocket;
        use std::net::UdpSocket;
        ...
        rt.block_on(async {
            ...
            let target = SelectedSendTarget::new(send_sock, None, recv_addr);
            let target_key = target.key();
            let flow = LinuxSequencedSendFlow::spawn(target_key, linux_batch_now_ms());

            flow.complete(
                1,
                LinuxSendBatchItem::Packets(vec![sealed(&target, target_key, b"second")]),
            );
            std::thread::sleep(std::time::Duration::from_millis(20));
            flow.complete(
                0,
                LinuxSendBatchItem::Packets(vec![sealed(&target, target_key, b"first")]),
            );

            let mut buf = [0u8; 64];
            let (len, _) = recv_sock.recv_from(&mut buf).expect("recv first");
            assert_eq!(&buf[..len], b"first");
            let (len, _) = recv_sock.recv_from(&mut buf).expect("recv second");
