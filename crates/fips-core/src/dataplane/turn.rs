...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct DataplaneFspTunPacketCommit {
    source_addr: NodeAddr,
    previous_hop_addr: NodeAddr,
    received_k_bit: bool,
    direct_path: bool,
...
impl DataplaneFspTunPacketCommit {
    fn new(source_addr: NodeAddr, previous_hop_addr: NodeAddr, received_k_bit: bool) -> Self {
        Self {
            source_addr,
            previous_hop_addr,
            received_k_bit,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct DataplaneFspTunPacketCommitRun {
    commit: DataplaneFspTunPacketCommit,
    len: usize,
}

impl DataplaneFspTunPacketCommitRun {
    fn new(commit: DataplaneFspTunPacketCommit, len: usize) -> Self {
        Self { commit, len }
    }

    pub(crate) fn commit(&self) -> DataplaneFspTunPacketCommit {
        self.commit
    }
    ...
    fn try_extend(&mut self, commit: DataplaneFspTunPacketCommit, len: usize) -> bool {
        if self.commit != commit {
            return false;
        }
        self.len = self.len.saturating_add(len);
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct DataplaneFspTunPacket {
    source_addr: NodeAddr,
    ce_flag: bool,
    packet: PacketBuffer,
}

impl DataplaneFspTunPacket {
    pub(crate) fn source_addr(&self) -> NodeAddr {
        self.source_addr
    }
    ...
    pub(crate) fn into_packet(self) -> PacketBuffer {
        self.packet
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct DataplaneFspTunPacketIngress {
    commit: DataplaneFspTunPacketCommit,
    msg_type: u8,
    body_len: usize,
    receive_sync: FspReceiveSync,
    activity_tick: Option<ActivityTick>,
    packet: Option<DataplaneFspTunPacket>,
}

impl DataplaneFspTunPacketIngress {
    fn from_output(output: PacketOutput) -> Result<Self, PacketOutput> {
        let OutputTarget::SessionPayload { local_addr } = output.target() else {
            return Err(output);
        };
        let source_addr = output.owner().node_addr();
        let Some(source_peer) = output.source_peer() else {
        ...
        let path_mtu = output.path_mtu();
        let activity_tick = output.activity_tick;
        let (timestamp_ms, msg_type, inner_flags, body_len) = {
            let Some(plaintext) = output.opened_payload() else {
                return Err(output);
            };
            let Some((timestamp_ms, msg_type, inner_flags, body)) =
            ...
            if msg_type != crate::protocol::SessionMessageType::DataPacket.to_byte() {
                return Err(output);
            }
            ...
            (timestamp_ms, msg_type, inner_flags, body.len())
        };
        let receive_sync = FspReceiveSync {
            counter: output.counter(),
            received_k_bit: header.flags() & crate::node::session_wire::FSP_FLAG_K != 0,
            timestamp: timestamp_ms,
            plaintext_len: body_len.saturating_add(FSP_INNER_HEADER_SIZE),
            ce_flag,
            path_mtu,
            spin_bit: inner_flags & 0x01 != 0,
        };
        ...
        let shim_offset =
            FSP_INNER_HEADER_SIZE.saturating_add(crate::node::session_wire::FSP_PORT_HEADER_SIZE);
        ...
        let dst_ipv6 = crate::FipsAddress::from_node_addr(&local_addr)
            .to_ipv6()
            .octets();
        let packet = if crate::upper::ipv6_shim::decompress_ipv6_packet_buffer(
            &mut payload,
            shim_offset,
            src_ipv6,
            dst_ipv6,
        ) {
            Some(DataplaneFspTunPacket {
                source_addr,
                ce_flag,
                packet: payload,
            })
        ...
        Ok(Self {
            commit: DataplaneFspTunPacketCommit::new(
                source_addr,
                previous_hop_addr,
                receive_sync.received_k_bit,
            ),
    ...
    fn into_packet(self) -> Option<DataplaneFspTunPacket> {
        self.packet
    }
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub(crate) struct DataplaneFspTunPacketBatch {
    commit_runs: Vec<DataplaneFspTunPacketCommitRun>,
    packets: Vec<DataplaneFspTunPacket>,
    len: usize,
}

impl DataplaneFspTunPacketBatch {
    pub(crate) fn from_ingress(ingress: DataplaneFspTunPacketIngress) -> Self {
        let len = ingress.len();
        let commit = ingress.commit();
        let mut packets = Vec::new();
        if let Some(packet) = ingress.into_packet() {
            packets.push(packet);
        }
        Self {
            commit_runs: vec![DataplaneFspTunPacketCommitRun::new(commit, len)],
            packets,
            len,
        }
    }
    ...
    pub(crate) fn push(&mut self, ingress: DataplaneFspTunPacketIngress) {
        let len = ingress.len();
        let commit = ingress.commit();
        if !self
            .commit_runs
        ...
        {
            self.commit_runs
                .push(DataplaneFspTunPacketCommitRun::new(commit, len));
        }
        if let Some(packet) = ingress.into_packet() {
    ...
    pub(crate) fn commit_runs(&self) -> &[DataplaneFspTunPacketCommitRun] {
        &self.commit_runs
    }

    pub(crate) fn into_packets(self) -> Vec<DataplaneFspTunPacket> {
        self.packets
    }
}
