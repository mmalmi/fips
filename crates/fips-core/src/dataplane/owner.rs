...
struct OwnerRetireSlot {
    ...
    next_result: usize,
    open_fsp_session_payload: Option<bool>,
}
...
impl OwnerRetireSlot {
    ...
    fn remaining(&self) -> usize {
        self.slot.len().saturating_sub(self.next_result)
    }
    ...
    fn is_open_fsp_session_payload_run(&mut self) -> bool {
        *self
            .open_fsp_session_payload
            .get_or_insert_with(|| self.slot.is_open_fsp_session_payload_run())
    }
    ...
    fn drain_results(
        ...
        let drained = self.slot.drain_results(self.next_result, limit, consume);
        self.next_result = self.next_result.saturating_add(drained);
        drained
    }
}
...
pub(crate) struct OwnerState {
    ...
    pending: BTreeMap<OrderToken, OwnerRetireSlot>,
}
...
impl OwnerState {
    ...
    fn stage_retire_slot(&mut self, slot: Arc<CryptoReadySlot>) -> bool {
        ...
        let previous = self
            .pending
            .insert(slot.first_order(), OwnerRetireSlot::new(slot));
        assert!(previous.is_none(), "owner retire slot order reused");
        was_empty
    }
    ...
    fn has_ready_retirement(&self) -> bool {
        self.pending
            .get(&OrderToken(self.next_retire))
            .is_some_and(OwnerRetireSlot::is_ready)
    }
    ...
    fn retire_ready_slots_into(
        ...
        while retired_count < limit {
            ...
            let Some(mut slot) = self.pending.remove(&order) else {
                break;
            };
            if !slot.is_ready() {
                self.pending.insert(order, slot);
                break;
            }
            ...
            if !slot.is_empty() {
                let previous = self
                    .pending
                    .insert(OrderToken(self.next_retire), slot);
