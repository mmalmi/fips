...
#[test]
fn fsp_owner_report_only_traffic_quiesces_and_payload_resumes_measurement() {
    ...
    for mode in [MmpMode::Full, MmpMode::Lightweight, MmpMode::Minimal] {
        ...
        let exchange = |mover: &mut Dataplane, msg_type: SessionMessageType, counter: u64| {
            let outbound = OutboundPacket::fsp(
                owner,
                1,
                PacketClass::Mmp,
                0,
                PacketBuffer::new(vec![0; 12]),
            )
            .with_fsp_inner_header(msg_type.to_byte(), 0)
            .with_activity_tick(ActivityTick::new(1_001 + counter));
            mover.submit_outbound_packet(outbound).unwrap();
            assert_eq!(dispatch_outbound_available(mover, 8).len(), 1);
            let sync = FspReceiveSync {
                counter,
                received_k_bit: false,
                timestamp: counter as u32,
                plaintext_len: FSP_INNER_HEADER_SIZE + 12,
                ce_flag: false,
                path_mtu: u16::MAX,
                spin_bit: false,
            };
            assert!(
                mover
                    .record_authenticated_fsp_session(DataplaneAuthenticatedFspSession::new(
                        ...
                        12,
                        sync,
                        Some(ActivityTick::new(1_001 + counter)),
                        start + std::time::Duration::from_millis(counter),
                    ),)
                    .is_some()
            );
        };

        exchange(&mut mover, SessionMessageType::SenderReport, 0);
        exchange(&mut mover, SessionMessageType::ReceiverReport, 1);
        assert!(
            mover
                .collect_fsp_mmp_reports(start + std::time::Duration::from_secs(10))
                .reports
                .is_empty(),
            "{mode}: measuring reports must not produce more reports",
        );

        // Keep all wire counters, including the preceding reports, so quieting
        // report generation does not create artificial packet loss.
        exchange(&mut mover, SessionMessageType::EndpointData, 2);
        let active = mover.collect_fsp_mmp_reports(start + std::time::Duration::from_secs(20));
        assert_eq!(
            active.reports.len(),
            match mode {
                MmpMode::Full => 2,
        ...
        for report in active.reports {
            if report.msg_type == SessionMessageType::SenderReport.to_byte() {
                let sender = crate::protocol::SessionSenderReport::decode(&report.encoded).unwrap();
                assert_eq!(sender.cumulative_packets_sent, 3);
                assert_eq!(sender.interval_start_counter, 0);
                assert_eq!(sender.interval_end_counter, 2);
            } else {
                let receiver =
                    crate::protocol::SessionReceiverReport::decode(&report.encoded).unwrap();
                assert_eq!(receiver.cumulative_packets_recv, 3);
                assert_eq!(receiver.highest_counter, 2);
                assert_eq!(receiver.max_burst_loss, 0);
            }
        }

        exchange(&mut mover, SessionMessageType::SenderReport, 3);
        exchange(&mut mover, SessionMessageType::ReceiverReport, 4);
        assert!(
            mover
                .collect_fsp_mmp_reports(start + std::time::Duration::from_secs(30))
                .reports
                .is_empty(),
            "{mode}: reports generated by payload must settle again",
        );
    }
}
...
include!("aead_owner_activity/pending_epochs.rs");
