...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn explicit_network_rebind_moves_adopted_carrier_and_preserves_port() {
    ...
    let after = transport.send_snapshot().unwrap();
    assert_eq!(
        before_addr, after.local_addr,
        "the replacement must preserve the local NAT-traversal port"
    );
