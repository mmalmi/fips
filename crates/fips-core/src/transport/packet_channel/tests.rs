...
#[test]
fn packet_rx_bulk_ready_includes_pending_batch_tail() {
    let (tx, mut rx) = packet_channel(10);
    let addr = TransportAddr::from_string("test");
    ...
    assert_eq!(rx.bulk_ready_packets(), 3);
    assert_eq!(rx.try_recv().unwrap().data[0], 0xaa);
    assert_eq!(
