...
impl PacketMover2DirectFspTransportSegments {
    ...
    #[cfg(test)]
    fn materialize_packet_outputs(&self) -> Vec<PacketOutput> {
        let mut outputs = Vec::with_capacity(self.segments.len());
        for index in 0..self.segments.len() {
            ...
            outputs.push(packet_output_with_payload(&self.output, payload.into()));
        }
        outputs
    }
}
